...
namespace NeosProLauncher
{
	...
	public static class KeyVerification
	{
		...
		{
			string str = KeyVerification.GetHardwareString();
			str = Convert.ToBase64String(KeyVerification.GetHash(str));
		...
		{
			return KeyVerification.BaseId() + KeyVerification.CpuId() + KeyVerification.BiosId();
		}
		...
		{
			return KeyVerification.Identifier(wmiClass, new List<string> { property });
		}
		...
		{
			return KeyVerification.Identifier("Win32_Processor", new List<string> { "UniqueId", "ProcessorId", "Name", "Manufacturer" });
		}

		// Token: 0x06000014 RID: 20 RVA: 0x00002274 File Offset: 0x00000474
		private static string BiosId()
		{
			return KeyVerification.Identifier("Win32_BIOS", new List<string> { "Manufacturer", "SMBIOSBIOSVersion", "IdentificationCode", "SerialNumber", "ReleaseDate", "Version" });
		}

		// Token: 0x06000015 RID: 21 RVA: 0x000022D2 File Offset: 0x000004D2
		private static string BaseId()
		{
			return KeyVerification.Identifier("Win32_BaseBoard", new List<string> { "Model", "Manufacturer", "Name", "SerialNumber" });
		}
	}
}
